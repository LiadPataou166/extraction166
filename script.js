...
class ProductManager {
    ...
    async addProduct(product) {
        ...
        if (success) {
            ...
            // יצירת עמוד קטגוריה אוטומטי
            createCategoryPage(categoryData).then(pageCreated => {
                if (pageCreated) {
                    console.log('עמוד קטגוריה נוצר בהצלחה:', categoryData.name);
                }
            }).catch(err => {
                console.error('שגיאה ביצירת עמוד קטגוריה:', err);
            });
...
// ליצירת עמוד קטגוריה משופר עם עיצוב תואם לאתר
async function createCategoryPage(categoryData) {
    try {
        ...
        // קריאה לקובץ ה-CSS של האתר כדי להשתמש בסגנונות הקיימים
        let cssContent = "";
        try {
            ...
            console.warn('Could not fetch CSS file:', e);
        }
...
<head>
    ...
    <!-- SEO Meta Tags -->
    <meta name="keywords" content="${categoryData.seoKeywords || name + ', מוצרים, קנייה, איכות'}">
    <meta name="author" content="Doctor Instraction">
    ...
    <!-- אתחול הסגנונות הקיימים -->
    <link rel="stylesheet" href="style.css">

    <!-- סגנונות משלימים לעמוד קטגוריה -->
    <style>
        /* סגנונות ספציפיים לעמוד הקטגוריה */
        .category-banner {
            ...
            border-radius: 15px;
            overflow: hidden;
            border: 1px solid rgba(62, 255, 139, 0.1);
        }
        ...
        .category-banner::before {
            ...
            width: 100%;
            height: 100%;
            background: radial-gradient(circle at top right, rgba(62, 255, 139, 0.15), transparent 60%);
            pointer-events: none;
        }
        ...
        .category-banner h1::after {
            ...
            bottom: -10px;
            left: 0;
            box-shadow: 0 0 10px rgba(62, 255, 139, 0.5);
        }
        ...
        .category-banner i {
            ...
            text-shadow: 0 0 10px rgba(62, 255, 139, 0.5);
        }
        ...
        .filter-btn.active, .filter-btn:hover {
            ...
            box-shadow: 0 0 15px rgba(62, 255, 139, 0.3);
        }
        ...
        .sort-select:focus {
            outline: none;
            border-color: var(--primary-color);
            box-shadow: 0 0 0 2px rgba(62, 255, 139, 0.2);
        }
